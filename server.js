...
apiRouter.route('/movies/:movie_id')
	.get(function(req, res){
		...
		function(err, movie){
			...
			displayMovies += movie;
			var reviews = req.query.visible;
			var movieTitle = req.params.movie_id;
			if(visible == true){
				Review.find({movie_id: movieTitle}
					function(err, data){
						if(err)
							res.send(err);
						displayMovies += data;
	...
	.put(function(req, res){
		...
		function(err, movie){
			...
			if(req.body.review == true) review.get();
			movie.save(function(err){
				if(err) res.send(err);
